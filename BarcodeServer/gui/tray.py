...
class SystemTrayApp:
    ...
    def _init_tray(self):
        ...
        # 使用默认字体绘制，避免 font=None 导致 emoji 渲染异常
        try:
            font = ImageFont.load_default()
            draw.text((20, 28), "B", font=font, fill=(255, 255, 255, 255))
        except Exception:
            pass
        ...
        import pystray
        menu = pystray.Menu(
            pystray.MenuItem('[WIN] 显示主界面', on_open, default=True),
            pystray.Menu.SEPARATOR,
            pystray.MenuItem(lambda item: self.server.get_status_text(), None, enabled=False),
            pystray.MenuItem('[RST] 重启服务器', on_restart),
            pystray.Menu.SEPARATOR,
            pystray.MenuItem('[EXP] 打开导出目录', on_open_export),
            pystray.MenuItem('[DAT] 打开数据目录', on_open_data),
            pystray.Menu.SEPARATOR,
            pystray.MenuItem('[X] 退出', on_quit),
        )

        self.tray_icon = pystray.Icon('barcode-server', image, '[B] 包裹管理系统', menu)
    ...
    def _restart_server(self):
        # 直接重启，不再调用外部回调（避免双重重启）
        result = self.server.restart()
        try:
            from plyer import notification
            notification.notify(
                title='[B] 包裹管理系统',
                message='[OK] 服务器重启成功' if result else '[FAIL] 服务器重启失败',
                timeout=3,
            )
        except:
            pass
